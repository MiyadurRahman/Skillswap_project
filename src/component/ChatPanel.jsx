...
export const ChatPanel = ({ conversation, peer, myUid, myName, messages, onSend, onClose, onMarkRead }) => {
  ...
  useEffect(() => {
    if (conversation?.id && (conversation.unread?.[myUid] || 0) > 0) {
      onMarkRead?.(conversation.id, myUid);
    }
  }, [conversation?.id, myUid, onMarkRead]);
  ...
  const peerUid = peer?.uid || conversation?.participantIds?.find((id) => id !== myUid) || '';
